...
from model_mm import memory_model
...
def parse_opts():
    ...
    parser.add_argument('--memory',          default='train', type=str, help='')
...
class VideoDatasetTest(Dataset):
	...
	def __getitem__(self, idx):
		cla = os.path.split(os.path.split(self.ids[idx])[0])[1]
		num = random.randint(201, 210) 
		path = ("  " + cla + "\\" + cla + "_(" + str(num) + ")") 
        
		pathCTimgs = glob.glob(self.ids[idx]+"  ")
		pathWSIimgs = glob.glob(path+" ") # png
		pathCTimgs = pathCTimgs[:opt.timesteps]
		pathWSIimgs = pathWSIimgs[:opt.timesteps]
		label = labels_dict[self.labels[idx]]
...
memory = memory_model(opt).to(device)
...
if __name__=='__main__':  
	...
	for epoch in range(opt.num_epochs):
		...
		for i, (img,label) in enumerate(train_dl):
		    ...
		    CT1, CT2, CT3, _, CT_out = model_CT(CT)
		    WSI1, WSI2, WSI3, _, WSI_out = model_WSI(WSI)
		    MM1, MM2, MM3 = memory(WSI1.detach(), WSI2.detach(), WSI3.detach(), label)
		    
		    _,_,_,FUS_out = model(CT1.detach(), CT2.detach(), CT3.detach(), WSI1.detach(), WSI2.detach(), WSI3.detach())
		...
		with torch.no_grad():
		    for j, (img_test, label_test) in enumerate(test_dl):
		        ...
		        tMM1, tMM2, tMM3 = memory(tWSI1, tWSI2, tWSI3, label_test)
		...
		if test_acc_FUS > best_test_acc_FUS:
		    torch.save(model, ' ') # ./models/TPM.pth
